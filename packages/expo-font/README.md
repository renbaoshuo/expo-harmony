...
为 HarmonyOS 上的 React Native 应用提供 Expo Font 的原生实现，支持在运行时加载和注册自定义字体、将文字渲染为图片，并通过配置插件打包字体资源，与官方同版本的 expo-font 配套使用。

如果需要在预构建时将字体打包到 HarmonyOS 应用中，必须在 `app.json` 的 `plugins` 中传入 `@expo-harmony/expo-font`：
...
仅通过 `Font.loadAsync` 在运行时动态加载字体时，不需要在 `app.json` 中配置字体路径；Config Plugin 只负责预先打包并注册应用内字体资源。
