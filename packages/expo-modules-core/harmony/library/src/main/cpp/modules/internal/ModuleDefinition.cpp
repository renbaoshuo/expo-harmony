...
void NativeSharedObject::unbindFromRuntime(
    const RuntimeContext *context,
    long objectId) noexcept {
  std::scoped_lock lock(runtimeBindingMutex_);
  if (!runtimeBinding_.unbind(context, objectId)) {
    return;
  }
...
bool NativeSharedObject::isBoundToRuntime(
    const RuntimeContext *context,
...
bool NativeSharedObject::isReleaseRequested() const noexcept {
  return releaseRequested_.load(std::memory_order_acquire);
}
...
ModuleDefinitionBuilder &ModuleDefinitionBuilder::onStartObserving(
  ...
  definition_.startObservers.push_back(std::move(body));
  return *this;
}

ModuleDefinitionBuilder &ModuleDefinitionBuilder::onStopObserving(
    std::function<void(RuntimeContext &, const std::string &)> body) {
  definition_.stopObservers.push_back(std::move(body));
  return *this;
}
...
void validateModuleDefinition(const ModuleDefinition &definition) {
  ...
  for (const auto &klass : definition.classes) {
    ...
    if (klass.baseClassName.empty()) {
      throw CodedError(
          "ERR_INVALID_DEFINITION",
          owner + " must derive from SharedObject, SharedRef, or another registered Expo class.");
    }
