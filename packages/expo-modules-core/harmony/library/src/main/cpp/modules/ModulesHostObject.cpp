...
bool shouldNotifyObserver(
    ...
    bool starting) {
  if (observer.everyEvent) return true;
  if (observer.eventName) return *observer.eventName == eventName;
...
std::vector<const ClassDefinition*> orderedClasses(
  ...
  while (result.size() < module.classes.size()) {
    ...
    for (const auto& candidate : module.classes) {
      if (completed.contains(candidate.name)) continue;
      ...
      const bool nativeBase =
          baseClass == "SharedObject" || baseClass == "SharedRef";
      ...
      if (!nativeBase && localBase && !completed.contains(baseClass)) continue;
...
void defineFunction(
  ...
  if (definition.enumerable) {
    ...
    expo::common::defineProperty(runtime, &target, definition.name.c_str(), {
        .configurable = false,
        .enumerable = false,
        .writable = false,
...
void defineProperty(
  ...
  expo::common::defineProperty(runtime, &target, definition.name.c_str(), {
      ...
      .get = [context, path, definition = &definition](
      ...
      .set = definition.setter
          ? std::function<void(jsi::Runtime&, jsi::Object, jsi::Value)>(
                [context, path, definition = &definition](
                    jsi::Runtime& rt,
                    jsi::Object receiver,
                    jsi::Value value) {
...
void defineConstant(
  ...
  auto cachedValue =
      std::make_shared<std::unique_ptr<jsi::Value>>();
  expo::common::defineProperty(runtime, &target, name.c_str(), {
      .configurable = false,
      .enumerable = true,
      .get = [context,
              path = std::move(path),
              factory = &factory,
              cachedValue](jsi::Runtime& rt, jsi::Object) {
        if (*cachedValue) {
          ...
          [context,
           ...
           definition = &definition](
            ...
            return translateErrors(rt, context, path, definition->async, [&]() {
              ...
              if (!definition->async) return result;
              if (result.isObject()) {
...
void defineSharedProperty(
  ...
  expo::common::defineProperty(runtime, &target, definition.name.c_str(), {
      .configurable = false,
      .enumerable = definition.enumerable,
      .get = [context, path, moduleName, className, definition = &definition](
                 jsi::Runtime& rt, jsi::Object receiver) {
        return translateErrors(rt, context, path, false, [&]() {
      ...
      .set = definition.setter
          ? std::function<void(jsi::Runtime&, jsi::Object, jsi::Value)>(
                [context, path, moduleName, className, definition = &definition](
                    jsi::Runtime& rt,
                    jsi::Object receiver,
                    jsi::Value value) {
                  translateErrors(rt, context, path, false, [&]() {
...
jsi::Value ModulesHostObject::get(
  ...
  if (!context_->isAlive()) return jsi::Value::undefined();
  ...
  if (!definition) return jsi::Value::undefined();
...
jsi::Object ModulesHostObject::createModule(
  ...
  expo::common::defineProperty(runtime, &module, "__expo_module_name__", {
      .value = jsi::String::createFromUtf8(runtime, definition.name)});
             ...
             observedEventCount](
              ...
              if (count == 1 && arguments[0].isString()) {
                ...
                for (const auto& observer : definition->startObservers) {
                  if (observer.body && shouldNotifyObserver(
                          observer, eventName, *observedEventCount, true)) {
        ...
        jsi::Function::createFromHostFunction(
            ...
            [context = context_,
             ...
             observedEventCount](
              ...
              if (count == 1 && arguments[0].isString()) {
                const auto eventName = arguments[0].getString(rt).utf8(rt);
                if (*observedEventCount > 0) --*observedEventCount;
                for (const auto& observer : definition->stopObservers) {
                  if (observer.body && shouldNotifyObserver(
                          observer, eventName, *observedEventCount, false)) {
                    observer.body(*context, eventName);
                  }
                }
              }
  ...
  for (const auto* classDefinitionPointer : orderedClasses(definition)) {
      ...
      if (!baseValue.isObject() || !baseValue.getObject(runtime).isFunction(runtime)) {
        throw makeJSError(
            ...
            "Base class '" + baseModuleName + "." + baseClassName +
                "' must be registered before '" + classPath + "'.");
      }
    ...
    if (!classDefinition.startObservers.empty() ||
        !classDefinition.stopObservers.empty()) {
      auto startName = jsi::PropNameID::forAscii(
          runtime, "__expo_onStartListeningToEvent");
               ...
               classDefinition = &classDefinition](
                ...
                if (count == 1 && arguments[0].isString()) {
                  ...
                  auto nativeObject = context->getNativeSharedObject(
                      ...
                      classDefinition->name);
                  const auto eventName = arguments[0].getString(rt).utf8(rt);
                  const auto observedEventCount =
                      context->beginObservingSharedObject(objectId);
              ...
              [context = context_,
               ...
               classDefinition = &classDefinition](
                ...
                if (count == 1 && arguments[0].isString()) {
                  ...
                  const auto eventName = arguments[0].getString(rt).utf8(rt);
                  const auto observedEventCount =
                      context->endObservingSharedObject(objectId);
    ...
    if (classDefinition.baseClassName == "SharedRef") {
      expo::common::defineProperty(runtime, &prototype, "nativeRefType", {
          .configurable = false,
          .enumerable = true,
          .get = [context = context_,
                  moduleName = definition.name,
