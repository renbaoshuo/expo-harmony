...
std::array<uint8_t, 20> sha1(const std::vector<uint8_t> &input) {
  ...
  for (size_t offset = 0; offset < data.size(); offset += 64) {
    for (size_t index = 0; index < 16; ++index) {
      ...
      words[index] = (static_cast<uint32_t>(data[position]) << 24U) | (static_cast<uint32_t>(data[position + 1]) << 16U) | (static_cast<uint32_t>(data[position + 2]) << 8U) | static_cast<uint32_t>(data[position + 3]);
    }
...
void applyUuidVersionAndVariant(std::array<uint8_t, 16> &bytes, uint8_t version) {
  if (version > 0x0fU) {
    throw std::invalid_argument("UUID version must fit in four bits");
  }
