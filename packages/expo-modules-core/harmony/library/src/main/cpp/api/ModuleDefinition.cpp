    ...
    throw CodedError(
        ...
        path_ + " expected " +
            (required == desired
                 ...
                 : std::to_string(required) + " to " + std::to_string(desired)) +
            " arguments, received " + std::to_string(argumentCount_) + ".");
  }
}
...
ModuleDefinitionBuilder& ModuleDefinitionBuilder::view(
  ...
  if (definition.componentName.empty()) {
    ...
    if (!defaultView) definition.componentName += "_" + definition.name;
  }
  if (definition.prototypeName.empty()) {
    definition.prototypeName = definition_.name;
    if (!defaultView) definition.prototypeName += "_" + definition.name;
  }
...
ModuleDefinitionBuilder& ModuleDefinitionBuilder::onStartObserving(
  ...
  definition_.startObservers.push_back({
      .everyEvent = true,
      .body = std::move(body)});
  return *this;
}
...
ModuleDefinitionBuilder& ModuleDefinitionBuilder::onStopObserving(
    std::function<void(RuntimeContext&, const std::string&)> body) {
  definition_.stopObservers.push_back({
      .everyEvent = true,
      .body = std::move(body)});
  return *this;
}
...
void validateFunctions(
  ...
  for (const auto& function : functions) {
    if ((!function.async && !function.body) ||
        (function.async && !function.body && !function.asyncBody)) {
      throw CodedError(
    ...
    if (!function.async && function.queue != FunctionQueue::JavaScript) {
      throw CodedError(
          "ERR_INVALID_DEFINITION",
          owner + "." + function.name +
              " is synchronous and must run on the JavaScript queue.");
    }
    if (function.async && !function.asyncBody &&
        function.queue != FunctionQueue::JavaScript) {
      throw CodedError(
          "ERR_INVALID_DEFINITION",
          owner + "." + function.name +
              " returns a JavaScript Promise/thenable directly and must run on the JavaScript queue.");
    }
...
void validateModuleDefinition(const ModuleDefinition& definition) {
  ...
  for (const auto& function : definition.functions) requireUniqueMember(function.name);
  for (const auto& property : definition.properties) requireUniqueMember(property.name);
  for (const auto& object : definition.objects) requireUniqueMember(object.name);
  for (const auto& klass : definition.classes) requireUniqueMember(klass.name);
  ...
  for (const auto& klass : definition.classes) {
    ...
    const bool nativeBacked =
        klass.nativeType != std::type_index(typeid(void));
    ...
    if (!nativeBacked &&
        (klass.baseClassName == "SharedRef" ||
         !klass.startObservers.empty() ||
         !klass.stopObservers.empty() ||
         (!klass.baseClassName.empty() &&
          klass.baseClassName != "SharedObject"))) {
      throw CodedError(
          "ERR_INVALID_DEFINITION",
          owner + " cannot use native SharedRef/custom inheritance or observers.");
    }
    const auto constructorRequiredArity =
        klass.constructorRequiredArity.value_or(klass.constructorArity);
    ...
    for (const auto& function : klass.functions) {
      if (!instanceMembers.insert(function.name).second) {
        throw CodedError(
            "ERR_DUPLICATE_DEFINITION",
            definition.name + "." + klass.name +
                " defines instance member '" + function.name + "' more than once.");
      }
    }
    for (const auto& property : klass.properties) {
      if (!instanceMembers.insert(property.name).second) {
        throw CodedError(
            "ERR_DUPLICATE_DEFINITION",
            definition.name + "." + klass.name +
                " defines instance member '" + property.name + "' more than once.");
      }
    }
    ...
    for (const auto& function : klass.staticFunctions) {
      if (!staticMembers.insert(function.name).second) {
        throw CodedError(
            "ERR_DUPLICATE_DEFINITION",
            definition.name + "." + klass.name +
                " defines static member '" + function.name + "' more than once.");
      }
    }
    for (const auto& property : klass.staticProperties) {
      if (!staticMembers.insert(property.name).second) {
        throw CodedError(
            "ERR_DUPLICATE_DEFINITION",
            definition.name + "." + klass.name +
                " defines static member '" + property.name + "' more than once.");
      }
    }
    ...
    for (const auto& function : klass.functions) {
      if ((!function.async && !function.body) ||
        ...
        throw CodedError(
            "ERR_INVALID_DEFINITION",
            definition.name + "." + klass.name + "." + function.name +
                " has no native body.");
      }
      if (!function.async && function.queue != FunctionQueue::JavaScript) {
        throw CodedError(
      ...
      if (requiredArity > function.arity) {
        throw CodedError(
            "ERR_INVALID_DEFINITION",
            definition.name + "." + klass.name + "." + function.name +
                " has an invalid required argument count.");
      }
    }
    ...
    if (nativeBacked &&
        (!klass.javaScriptFunctions.empty() ||
         !klass.javaScriptProperties.empty() ||
         klass.javaScriptConstructor)) {
      throw CodedError(
          "ERR_INVALID_DEFINITION",
  ...
  for (const auto& view : definition.views) {
    ...
    if (view.defaultView) ++defaultViewCount;
    if (view.componentName.empty() ||
        !componentNames.insert(view.componentName).second) {
      throw CodedError(
          "ERR_INVALID_DEFINITION",
          owner + " has an empty or duplicate Fabric component name.");
    }
    if (view.prototypeName.empty() ||
        !prototypeNames.insert(view.prototypeName).second) {
      throw CodedError(
          "ERR_INVALID_DEFINITION",
