...
class CodedError final : public std::runtime_error {
 public:
  ...
  const std::optional<ExceptionOrigin>& origin() const noexcept { return origin_; }
  const std::shared_ptr<const CodedError>& cause() const noexcept { return cause_; }
  const std::vector<std::string>& nativeStack() const noexcept { return nativeStack_; }
