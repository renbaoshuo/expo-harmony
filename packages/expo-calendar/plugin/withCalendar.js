...
function withHarmonyCalendar(config, options = {}) {
  const enabled = config.harmony?.bundleName || config.platforms?.includes('harmony');
  if (!enabled) return config;

  for (const name of ['calendarPermission', 'readCalendarPermission', 'writeCalendarPermission']) {
  ...
  const reasons = {
    expo_calendar_read_permission_reason: options.readCalendarPermission ?? options.calendarPermission,
    expo_calendar_write_permission_reason: options.writeCalendarPermission ?? options.calendarPermission,
  };
...
module.exports = createRunOncePlugin(withHarmonyCalendar, pkg.name, pkg.version);
