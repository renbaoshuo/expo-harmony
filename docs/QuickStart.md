...
- `bundleName` 是应用包名，必填。
- `targetSdkVersion` 和 `compatibleSdkVersion` 默认分别为 24（HarmonyOS 6.1.1）和 20（HarmonyOS 6.0.0）。通过 [expo-build-properties](../packages/expo-build-properties/README.md) 配置 SDK、ABI、SO 打包和 release 选项。应用的最低兼容版本须不低于所有原生依赖的要求：`expo-constants` 为 14、`@expo-harmony/expo` 为 20、`expo-file-system` 为 21，因此 demo 显式设置为 21。
...
不配置签名只会收到警告，不影响未签名构建。首次生成（项目里还没有 `harmony/` 目录）时也不校验签名文件：DevEco Studio 要先生成工程才能创建证书，此时 prebuild 按未签名处理并给出警告；工程生成之后，签名文件读不到就会报错并中止。要往真机装 Release 包，需要签名材料：证书、私钥库和 profile 文件，DevEco Studio 可以生成。把它们写成一个 JSON 文件，字段和 DevEco Studio 写进 `build-profile.json5` 的 `signingConfigs` 一致：`certpath`、`storeFile`、`profile`、`storePassword`、`keyAlias`、`keyPassword`、`signAlg`。
